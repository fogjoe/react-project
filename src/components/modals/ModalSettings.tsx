...
import { type ModalProps, type MenuProps, theme, ConfigProvider, Modal, Menu } from 'antd'
import { InfoIcon, ShirtIcon } from 'lucide-react'
import { create, useModal } from '@ebay/nice-modal-react'
import { Viewer } from '@bytemd/react'
import { ThemeEditor, useThemeContext } from '../ThemeEditor'

const enum SettingMenuKey {
  Appearance = '0',
  About = '1'
}

/**
 * when i use ':MenuProps['items']', it occurs the error:
 * Property 'label' does not exist on type 'MenuDividerType | MenuItemType | SubMenuType<MenuItemType> | MenuItemGroupType<MenuItemType>'.
 * Property 'label' does not exist on type 'MenuDividerType'
 */
const settingMenuItems = [
  {
    key: SettingMenuKey.Appearance,
    icon: <ShirtIcon size={16} />,
    label: '外观'
  },
  {
    key: SettingMenuKey.About,
...
const renderMenuContent = (props: { menuKey: SettingMenuKey }) => {
  switch (props.menuKey) {
    case SettingMenuKey.Appearance:
      return <ThemeEditorWrapper />
    case SettingMenuKey.About:
      return (
        <Viewer
...
export const ModalSettings = create((props: ModalSettingsProps) => {
  ...
  const [selectedKeys, setSelectedKeys] = useState<[SettingMenuKey]>([SettingMenuKey.Appearance])
            ...
            <div style={{ padding: `0 ${token.paddingMD}px` }}>
              ...
              >
                <Menu
                  ...
                  onClick={({ key }) => {
                    setSelectedKeys([key as SettingMenuKey])
                  }}
                />
              </ConfigProvider>
            </div>
