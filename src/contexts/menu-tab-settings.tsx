import { ApiTabItem, EditStatus } from "@/components/ApiTab";
...
interface MenuTabContextData {
  ...
  setActionTabKey: React.Dispatch<React.SetStateAction<ApiTabItem['key'] | undefined>>

  /** 上一次被激活的页签 */
  lastActiveTabKey: ApiTabItem['key'] | undefined
  setLastActionTabKey: React.Dispatch<React.SetStateAction<ApiTabItem['key'] | undefined>>
}
...
export function MenuTabProvider(props: React.PropsWithChildren) {
  ...
  const [activeTabKey, setActionTabKey] = useState<ApiTabItem['key']>()
  const [lastActiveTabKey, setLastActionTabKey] = useState<ApiTabItem['key']>()

  useEffect(() => {
    setTabItems(initialTabItems)
    setActionTabKey(initialActiveTabKey)
  }, [])

  return <MenuTabContext.Provider value={{
    tabItems,
    setTabItems,
    ...
    lastActiveTabKey,
    setLastActionTabKey
  }}>{props.children}</MenuTabContext.Provider>
}
...
export function useMenuTabHelpers(): MenuTabHelpers {
  ...
  const activeTabItem = useEvent<MenuTabHelpers['activeTabItem']>((payload) => {
    // setLastActionTabKey(() => activeTabKey)
    setLastActionTabKey(activeTabKey)

    tabItems.length > 0 && setActionTabKey(payload.key)
  })
  ...
  const addTabItem = useEvent<MenuTabHelpers['addTabItem']>((payload, { autoActive = true, replaceTab } = {}) => {
    ...
    if (isSameTabPresent) {
      ...
      if (replaceTab) {
        setTabItems(items => items.map(item => item.key === replaceTab ? payload : item))
      } else {
  ...
  const removeTabItem = useEvent<MenuTabHelpers['removeTabItem']>((payload) => {
    setTabItems((items) => {
      ...
      if (activeTabKey === payload.key) {
        setActionTabKey(() => undefined)

        const valideTabKey = lastActiveTabKey && newItems.findIndex(item => item.key === lastActiveTabKey) !== -1

        if (valideTabKey) {
          activeTabItem({ key: lastActiveTabKey })
        } else {
          setLastActionTabKey(() => undefined)

          const lastTabKey = newItems.at(-1)?.key

          lastTabKey && activeTabItem({ key: lastTabKey })
        }
      }
  ...
  const setTabItemEditStatus = useEvent<MenuTabHelpers['setTabItemEditStatus']>((payload, editStatus) => {
    setTabItems(items => {
